...
// at the historical Python research engine), preserving its
...
// Benchmark comparisons are shown only for synthetic sample artifacts.
...
public enum MemoBuilder {
    ...
    public static func build(
        ...
        let outcomeUnit = isSynthetic ? "lead" : "outcome"

        let best = scenarios.rows
            .filter { $0.pPositive >= 0.75 && $0.deltaLeadsWeekly.med > 0 && $0.shiftDollars > 0 }
            .max { $0.deltaLeadsWeekly.med < $1.deltaLeadsWeekly.med }

        var sections: [MemoSection] = []

        // "YouTube, Display, and Audio were optimistic... the agency planning
        // corrections" names two things specific to the CarePoint synthetic
        // twin: three of ITS channel labels, and ITS ad-buying vendor. A
        // package without those channels (i.e. not CarePoint) still got this
        ...
        var findings = "Across the fitted weekly data, the lowest modeled marginal costs per \(outcomeUnit) are in \(cheapNames). "
            + "\(expNames) carry the highest marginal cost per \(outcomeUnit) at reference spend."
        // These benchmarks belong to the bundled synthetic brief. A real
        // fit has no client-specific benchmark calibration to compare.
        if isSynthetic {
            findings += " Vendor benchmarks for those channels were optimistic relative to this market; "
        ...
        sections.append(MemoSection(heading: "What the model found", body: findings))
        ...
        if let best = best,
           ...
           let targetLabel = chans.first(where: { $0.key == best.target })?.label {
            decisionBody = "Shift \(best.shiftPct)% of weekly \(sourceLabel) spend "
                ...
                + "modeled lift \(formatSigned1(best.deltaLeadsWeekly.med)) \(outcomeUnit)s/week "
            ...
            decisionBody = "No budget shift clears the positive-lift and confidence requirements at a positive dollar amount. "
                + "Hold the current allocation and inspect the scenario intervals before making a change."
        }
        ...
        let trustCore = "The full and holdout fits report maximum R-hat \(diagnostics.rhatMax) and \(diagnostics.divergences) total divergences. "
            + "Across \(diagnostics.holdoutWeeks) holdout weeks, forecast error was "
            + "\(diagnostics.mapeHoldoutPct)% MAPE with \(diagnostics.coverage90Pct)% interval coverage."
        let trustBody = isSynthetic
            ? trustCore + " On synthetic data with hidden ground truth, recovered intervals contained the truth; the full receipt is on the Model Card."
            : trustCore
        sections.append(MemoSection(heading: "Model checks", body: trustBody))
        ...
        let limitsBody = isSynthetic
            ? "Weekly grain, last-click stance, and correlated channel activity. Lift tests and a control sheet "
                ...
                + "synthetic twin calibrated to the bundled sample brief; the identical pipeline retrains the day the real export lands."
            : "Weekly grain, last-click stance, and correlated channel activity. Lift tests and a control sheet "
        ...
        // run happens to be against the CarePoint synthetic twin: that case
        // gets its CarePoint identity by the caller actually passing
        // --label "CarePoint", not by this code detecting or assuming it.
        let kpiTitle = ChannelRegistry.titleCase(kpiName)
        return MemoArtifact(title: "\(packageLabel): \(kpiTitle) Measurement & Planning Memo", date: date, sections: sections)
    }
}
